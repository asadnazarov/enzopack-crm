-- finished_goods_movements.movement_date was a plain `date` column, so the
-- warehouse history lost the time of day. Switch it to `timestamptz` (any
-- existing rows keep midnight UTC, which is fine — they only had a date to
-- begin with) and default new rows to `now()`. update_order_status is
-- updated to insert `now()` explicitly instead of `current_date`.

alter table finished_goods_movements
  alter column movement_date type timestamptz using movement_date::timestamptz,
