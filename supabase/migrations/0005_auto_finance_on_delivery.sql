-- Automatically record an income entry in finance_transactions when an
-- order becomes "delivered" (the client is considered to have paid once
-- goods are delivered). If the status is later moved away from delivered
-- (now possible in either direction), the auto-generated entry is removed
-- again so the books stay consistent. Manually added transactions (any
-- other category) are never touched.
